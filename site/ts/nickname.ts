...
export const isValidNickname = (
  nickname: string,
  allowExcessLength: boolean = true,
) => {
  const nonEmptyCondition = nickname.trim().length > 0;
  const lengthCondition =
    allowExcessLength || nickname.length <= MAX_NICKNAME_LENGTH;
  const notXssCondition = !conatainsHtmlTags(nickname);

  // The regex tests for values NOT between 00 and 7F (the valid ASCII range)
  const asciiCondition = !/[^\x00-\x7F]/.test(nickname);

  const notSlurCondition =
    !/(n.g{1,}.(?:r)?|n[ei]g{1,}|f[aeiou]g{1,}[aeiou]t|f[au]g{1,})/i.test(
      nickname,
    );
...
export const receiveNickname = async (
  rejectOnInvalid: boolean = false,
): Promise<string> => {
  // Scroll to top
  window.scrollTo(0, 0);
  ...
  return new Promise((res, rej) => {
    ...
    nicknameDialogFormElement.onsubmit = (e) => {
      ...
      const recvNickname = nicknameDialogInputElement.value;
      const isInvalid =
        nicknameDialogInputElement.classList.contains("data-unbuyable") ||
        !isValidNickname(recvNickname);
      ...
      if (isInvalid && rejectOnInvalid) {
        ...
        return rej(
          "rejectOnInvalid was set to true and the given nickname was invalid.",
        );
      } else if (!isInvalid) {
        cleanup();
...
export const selectNickname = async (res: ServerSentWorkerData) => {
  nickname =
    res.results![0].nickname && isValidNickname(res.results![0].nickname)
